 ...
 *
 * 1. **Forwarder mode** (recommended) — Meta calls Laravel because the public
 *    HTTPS domain already terminates TLS. Laravel verifies the payload
 *    signature, then forwards the JSON body to the Node bot's `/inbound`
 *    endpoint (configured via `WHATSAPP_BOT_NOTIFY_URL`). The bot replies via
 *    the Graph API itself.
 *
 * 2. **Direct mode** — When `WHATSAPP_BOT_NOTIFY_URL` is empty, Laravel just
 *    logs the payload and returns 200. Use this when Meta is configured to
 *    POST directly to the Node bot host.
 */
class WhatsAppWebhookController extends Controller
{
    /**
     * Webhook verification handshake (Meta sends GET with hub.* query params).
     */
    ...
    /**
     * Inbound message callback. Meta requires a 200 response within 20s.
     */
    ...
    /**
     * Validate the X-Hub-Signature-256 header against `app_secret`.
     *
     * When `app_secret` is not configured (e.g. local dev), signature
     * verification is skipped so the webhook still works for smoke testing.
     */
