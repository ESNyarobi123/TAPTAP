<x-manager-layout>
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <button type="button" onclick="openAddTableModal()" class="min-h-[44px] inline-flex items-center justify-center gap-2 bg-linear-to-r from-fin-primary to-fin-primary-dark text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg hover:shadow-fin-primary/25 transition-all focus:outline-none focus:ring-2 focus:ring-fin-primary focus:ring-offset-2 focus:ring-offset-[#0f0a1e]">
    ...
    <div class="glass-card rounded-2xl p-8 mb-12 border-violet-500/30 bg-violet-500/5 relative overflow-hidden group">
        <div class="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
            <svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" class="text-fin-primary">
                <path d="M3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>
            </svg>
        </div>
        
        <div class="flex flex-col md:flex-row items-center gap-10 relative z-10">
            <div class="bg-surface-900 p-4 rounded-[2rem] shadow-2xl shadow-fin-primary/20">
                <img src="{{ whatsapp_branded_qr_url(Auth::user()->restaurant->whatsapp_qr_url, 200) }}" alt="Official QR" class="w-40 h-40">
            </div>
            
            <div class="flex-1 text-center md:text-left">
                <div class="inline-flex items-center px-3 py-1 rounded-full bg-violet-500/20 text-fin-primary text-[10px] font-bold uppercase tracking-widest mb-4">
                    Official Restaurant QR
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="glass-card rounded-2xl overflow-hidden card-hover group">
                <div class="p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div class="w-14 h-14 bg-linear-to-br from-fin-primary/15 to-cyan-500/10 rounded-xl flex items-center justify-center border border-violet-500/20 group-hover:scale-110 transition-transform">
                            <span class="text-2xl font-bold text-fin-primary">{{ $loop->iteration }}</span>
                        </div>
                        <div class="flex gap-1">
                            <button onclick="openEditTableModal({{ json_encode($table) }})" class="p-2 text-white/40 hover:text-fin-primary hover:bg-white/5 rounded-lg transition-all">
                            ...
                            <form action="{{ route('manager.tables.destroy', $table->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 text-white/40 hover:text-rose-600 hover:bg-rose-500/10 rounded-lg transition-all">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    @if($table->waiter)
                    <p class="text-xs font-medium text-fin-primary/80">Waiter: {{ $table->waiter->name }}</p>
                    @endif
                </div>
                ...
                <div class="bg-white/5 p-6 flex flex-col items-center gap-4 border-t border-white/5">
                    ...
                    <div class="w-full p-2 bg-white/5 rounded-lg border border-white/10 flex items-center justify-between mb-2">
                        ...
                        <div class="flex items-center gap-2">
                            <code class="text-sm font-mono font-bold text-cyan-600">{{ $table->table_tag }}</code>
                    ...
                    <div class="bg-surface-900 p-2 rounded-xl">
                        <img src="{{ whatsapp_branded_qr_url($table->whatsapp_qr_url, 150) }}" alt="QR Code" class="w-28 h-28">
                    </div>
                    <div class="flex gap-2 w-full">
                        <a href="{{ whatsapp_branded_qr_url($table->whatsapp_qr_url, 500) }}" download="table-{{ $table->id }}-qr.png" target="_blank" class="flex-1 glass py-2.5 rounded-xl font-semibold text-sm text-white/60 hover:text-white hover:bg-violet-600 transition-all flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/>
                            </svg>
                            Download
                        </a>
                        <button onclick="copyLink('{{ $table->whatsapp_qr_url }}', this)" class="flex-1 glass py-2.5 rounded-xl font-semibold text-sm text-white/60 hover:text-white hover:bg-cyan-600 transition-all flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>
                            </svg>
                            Copy Link
        ...
        <button onclick="openAddTableModal()" class="glass rounded-2xl border border-dashed border-white/20 flex flex-col items-center justify-center p-8 hover:border-violet-500 hover:bg-violet-500/10 transition-all group min-h-[320px]">
            ...
            <span class="font-semibold text-white/40 group-hover:text-fin-primary uppercase tracking-wider text-sm">Add New Table</span>
        </button>
    </div>
            ...
            <div class="p-6">
                ...
                <form id="tableForm" action="{{ route('manager.tables.store') }}" method="POST" class="space-y-4">
                    ...
                    <div id="waiterSelectWrap" class="hidden">
                        ...
                        <select name="waiter_id" id="tableWaiterId" class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl font-medium text-white focus:ring-2 focus:ring-fin-primary focus:border-transparent transition-all">
                    ...
                    <button type="submit" class="w-full bg-linear-to-r from-fin-primary to-fin-primary-dark text-white py-3.5 rounded-xl font-semibold hover:shadow-lg hover:shadow-fin-primary/25 transition-all mt-2">
                        Save Table
                    </button>
                </form>
            </div>
            ...
            try {
                ...
                if (button) {
                    ...
                    button.innerHTML = `
                        ...
                        Copied!
                    `;
                    button.classList.add('bg-emerald-500/20', 'border-emerald-500/30', 'text-emerald-600');
                    
                    setTimeout(() => {
                        button.innerHTML = originalContent;
                        button.classList.remove('bg-emerald-500/20', 'border-emerald-500/30', 'text-emerald-600');
                    }, 2000);
                } else {
