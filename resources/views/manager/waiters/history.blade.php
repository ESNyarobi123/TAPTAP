<x-manager-layout>
    ...
    <form method="GET" action="{{ route('manager.waiters.history') }}" class="glass-card rounded-2xl p-6 mb-8 border border-white/10">
        ...
        <div class="flex flex-wrap gap-4 items-end">
            ...
            <button type="submit" class="px-5 py-2.5 bg-gradient-to-r from-fin-primary to-fin-primary-dark text-white rounded-xl font-semibold hover:shadow-lg hover:shadow-violet-500/25 transition-all flex items-center gap-2">
    ...
    @if($assignmentHistory->isEmpty())
        <div class="glass-card py-16 text-center rounded-2xl border border-white/10">
            ...
            <a href="{{ route('manager.waiters.history') }}" class="inline-block mt-4 px-5 py-2.5 bg-white/10 text-white rounded-xl font-semibold hover:bg-white/20 transition-all">Clear filters</a>
        </div>
    @else
                            ...
                            <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    @if($a->employment_type === 'temporary')
                                        ...
                                        @endif
                                    @else
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold bg-white/10 text-white/60 border border-white/10">Long-term</span>
                                    @endif
                                </td>
